...
bool CRedisClient::TestOnline()
{
	...
	{//链接已经断开，开始重连计数，重连三次以后链接不上则认为此数据库已下线，将在client管理中进行剔除
		...
		if(ReConnect())
		{
			printf("CRedisClient::TestOnline::RedisClient Re Online! ip=%s, port=%d\n", m_address.c_str(), m_port);
			m_iReconnectCount = 0;
			return true;
		}
		else
		{
			...
			{
				printf("CRedisClient::TestOnline::RedisClient Off! ip=%s, port=%d\n", m_address.c_str(), m_port);
				return false;
			}
			else
			{
...
bool CRedisClient::OnConnect(struct timeval* tvl)
{
	...
	printf("CRedisClient::OnConnected success!\n");
	return true;
}
